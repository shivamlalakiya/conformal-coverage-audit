...
| **Boundary behaviour of every executable helper** | ✅ Complete, `probes/conformance_suite.py`. Below the feasibility floor **8 of 14 shipped helpers hand back a finite threshold; a 9th hands back a bare count. All 9 are silent.** 3 answer `+inf`, each with a warning, and 2 raise — two defensible answers that are not the same promise, so nothing ecosystem-wide exists for a helper to break |
| **Is the clip reachable through the public API?** | ✅ Complete, `probes/mapie_clip_reachability.py`. Yes, via `allow_infinite_bounds`, which skips the guard rather than changing what follows it. Whatever level is requested, what comes back is rank n, hence n/(n+1): 0.9091 against 0.95 at n=10, confirmed at 50k draws a cell. Reached by 3 public regressor classes. **This withdraws a retraction of our own** — the earlier scan held the flag fixed. Filed as MAPIE#980 |
| **Does the LAC crossval `quantiles_` decide anything?** | ✅ Complete, `probes/lac_crossval_dead_value.py`. No — scale it by 1000 and not one returned set moves. APS is the control: identical arithmetic, identical branch, and APS *does* consult it. The census anchor moved onto the line that actually resolves the level; the total stays 35 |
